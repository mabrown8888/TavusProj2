...
export const Debrief = ({ gote, onRunAgain, onNewScene }: DebriefProps) => {
  ...
  const handleAnalyze = async () => {
    ...
    setError(null);
    try {
      const result = await analyzeScene(gote, reflection);
      setAnalysis(result);
    } catch (err) {
  ...
  const goteFields = [
    { label: "G — Goal", value: gote.goal },
    { label: "O — Obstacles", value: gote.obstacles },
    { label: "T — Tactics", value: gote.tactics },
    { label: "E — End", value: gote.endCondition },
  ];
    ...
    <div className="min-h-screen bg-stage-bg flex flex-col items-center px-4 py-12">
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-stage-card border border-stage-border rounded-xl p-5 mb-6">
          ...
          <p className="text-stage-muted text-sm mb-4 italic">"{gote.scenarioDescription}"</p>
          <div className="grid grid-cols-1 gap-3">
            {goteFields.map(({ label, value }) => (
              <div key={label}>
        ...
        {!analysis ? (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-stage-muted text-xs mb-2">What moments stood out? Where did you struggle? What landed?</p>
              <textarea
                value={reflection}
                onChange={(e) => setReflection(e.target.value)}
                placeholder="The scene felt tense in the middle when the character started pushing harder..."
                rows={4}
