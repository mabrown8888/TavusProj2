...
const VideoPreview = memo(({ id }: { id: string }) => {
  const videoState = useVideoTrack(id);
  const w = videoState.track?.getSettings()?.width;
  const h = videoState.track?.getSettings()?.height;
  const isVertical = w && h ? w < h : false;

  return (
    <div className={`${styles.previewVideoContainer} ${isVertical ? styles.previewVideoContainerVertical : ""} ${videoState.isOff ? styles.previewVideoContainerHidden : ""}`}>
      <DailyVideo
        automirror
        sessionId={id}
        type="video"
  ...
  if (!replicaId) {
    return (
      <div className={styles.waitingContainer}>
        <p>Connecting to scene partner...</p>
      </div>
    );
  }
...
export const Conversation = memo(({ onLeave, conversationUrl }: ConversationProps) => {
        ...
        <div className={styles.footerControls}>
          ...
          <button type="button" className={styles.leaveButton} onClick={handleLeave}>
            <span className={styles.leaveButtonIcon}>
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" aria-label="End Scene">
